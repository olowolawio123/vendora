...
// ONLY ACTIVE USERS + APPROVED SELLERS
// ONLY PRODUCTS NOT HIDDEN BY SELLER SUSPENSION
// =====================================================
router.get("/", async (req, res) => {
  try {
    ...
    // =================================================
    // FIND SELLERS WHO ARE CURRENTLY ALLOWED
    // =================================================
    ...
    const validSellerIds = approvedSellers
      .filter(
        (seller) =>
          seller.user &&
    ...
    const products = await Product.find(filter)
      .populate(
        "seller",
        "storeName location rating status"
      )
      .sort(sortOption)
      .skip(skip)
      .limit(itemsPerPage);
    ...
    try {
      ...
      if (!seller) {
        return res.status(403).json({
          success: false,
          message:
            "Only approved sellers can access seller products",
        });
      }
...
// PUBLIC
// ONLY ACTIVE USER + APPROVED SELLER
// ONLY PRODUCTS NOT HIDDEN BY SELLER SUSPENSION
// =====================================================
router.get(
  "/:productId",
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      // Seller-controlled updates must
      // never remove the suspension flag.
      // Admin suspension/reactivation controls it.
      await product.save();
